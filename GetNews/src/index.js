...
function getWelcomeResponse(callback) {
    var speechOutput = "Welcome! Do you want to hear news from News API??"

    var reprompt = "Do you want to hear news from News API?"
...
function handleGetNewsIntent(intent, session, callback) {
    ...
    getJSON(intent.slots.source.value, function(data) {
        if (data != "ERROR") {
...
function url(source) {
    return {
        ...
        qs: {
            "apikey" : "e84ed447a3d1498180929c89f0565ee2",
            "source" : source
        }
    }
}
...
function getJSON(source, callback) {
    // HTTP - NewsAPI
    request.get(url(source), function(error, response, body) {
        var d = JSON.parse(body)
        // var result = d.query.searchinfo.totalhits
        // var result = d.articles[index].title
        var result = d.articles
        // console.log(result);
        if (!!result) {
